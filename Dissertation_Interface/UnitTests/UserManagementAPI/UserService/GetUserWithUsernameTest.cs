...
public class GetUserWithUsernameTest
{
    ...
    [Test]
    public async Task GetUserByUserName_ShouldReturnFailure_WhenUserIsNotFound()
    {
        // Arrange
        const string userName = "nonExistingUser";
        this._userManager.Setup(manager => manager.FindByNameAsync(userName))
            .ReturnsAsync((ApplicationUser)null!);

        // Act
        ResponseDto<GetUserDto> result = await this._userService.GetUserByUserName(userName);
    ...
    [Test]
    public async Task GetUserByUserName_ShouldReturnValidUserDto_WhenUserIsFound()
    {
        // Arrange
        var userName = "existingUser";
        var roles = new List<string> { "Role1", "Role2" };

        this._httpContextAccessor.Setup(x => x.HttpContext!.Items["UserId"]).Returns(userName);
        this._userManager.Setup(manager => manager.FindByNameAsync(userName)).ReturnsAsync(this._applicationUser);
        ...
        Assert.Multiple(() =>
        {
            ...
            // Further assert statements to verify Role, IsLockedOut etc.
        });
    }
}